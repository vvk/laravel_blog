...
@section('breadcrumb_button')
    <a href="{{url('admin/article/create')}}" class="btn btn-sm btn-success fa fa-edit"> 写文章</a>
@endsection
...
@section('content')
    ...
    <div class="panel-body">
        <div class="panel-body">
            <div class="btn-group">
                <a href="{{url('admin/article')}}" class="btn @if($status==0) btn-primary @else btn-default @endif">全部</a>
                @foreach(config('web.article_status') as $k=>$v)
            ...
            @if($data->items())
                @foreach($data as $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{url('admin/article/'.$item['id'].'/edit/')}}" class="fa fa-edit btn btn-primary btn-sm">&nbsp;修改</a>
                            <a href="{{url('archives').'/'.$item['id']}}?type=preview" target="_blank" class="fa fa-paper-plane-o btn btn-primary btn-sm">&nbsp;查看</a>
